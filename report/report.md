...
## Survival length per base

The two halves of the question, in which genomes and up to what k, combine into one number per base and genome: the longest exact k-mer starting at that base that still exists in that genome. We computed it on the k ladder (17 values from 16 to 2,001) for 64 regions totalling 6.9 Mb, chosen to cover the array boundaries, the satellite blocks, ordinary sequence and the identical segments, and Figure 4 draws seven of them.

![**Survival length of exact k-mers.** For each base (columns, 10 bp bins) and each genome (rows), the colour gives the longest k-mer starting at that base that exists in the genome, on a ladder from 16 to 2,001. Regions: ordinary non-repeat sequence; the chr1 active HOR array; the monomeric alpha-satellite layer beside the chr8 array; an HSat2 block; an HSat1B block; an HSat3 block; the ultraconserved element in the PAX6 region of chr11.](../fig/fig_survival.png){width=100%}

In ordinary sequence the median survival is 51 bases in chimpanzee, bonobo and gorilla, 25 in the orangutans and 21 in siamang, against 1,501 or more in the other human, the expected spacing of differences at the known divergences and heterozygosity. Inside the active HOR array of chr1 the median falls to 25 in the African apes and to 16, the shortest k tested, in the orangutans and siamang, uniformly across the array: no copy in any ape matches a human copy for more than a few dozen bases. The monomeric layer beside the chr8 array is patchier and older, with runs of 50 to 300 bases in gorilla and the orangutans. HSat2 survives to 21 bases in the African apes and to 16 in the others; HSat1B to 16 in every ape while the other human carries every 2 kb of it; HSat3 shows the same short survival in all six apes, the (GGAAT)$_n$ vocabulary and nothing longer. The PAX6-region element is the opposite: 1.1 kb over which every genome matches for 700 bases or more, flanked by sequence that decays to the ordinary 20 to 100 within a kilobase. The colour scale is the same in every panel; what changes between regions is the scale at which the genomes still agree.

## Divergence from k-mer survival

In sequence without repeats, an exact k-mer survives between two genomes when none of its k bases has changed on either lineage, so the shared fraction is close to $(1-d)^k$ for a per-base divergence $d$. Table 5 gives two alignment-free estimates against the published alignment-based values, and Figure 5 shows survival against divergence time for every class.
...
## Sharing by copy number: two clocks

The multiplicity of a k-mer in CHM13 changes what its presence elsewhere means. A single-copy 31-mer found in chimpanzee is evidence about one locus; a 31-mer with a thousand copies found in chimpanzee says that the repeat family predates the split, whatever happened at any one locus. Figure 7 separates the two. Outside the repeats, single-copy 31-mers (88% of the class) exist in chimpanzee 66% of the time and in siamang 30%, the survival expected from divergence, whereas 31-mers with ten or more copies exist in every ape 98 to 100% of the time and almost all fall in the deepest stratum: they are the transposable-element and simple-repeat words that every ape genome carries, and they inflate the class's sharing with siamang from 30% to 37%. In segmental duplications the single-copy 31-mers, the paralogue-distinguishing variants, are shared less than ordinary sequence (49% in chimpanzee, 17% in siamang, 34% human-only). Inside the active HOR arrays 85% of positions carry a 31-mer with 255 or more copies, and those are the ones shared with the apes (44% in chimpanzee, 6% in siamang); the 1.1% of positions with a single-copy 31-mer are 44% private to CHM13 and 45% human-only, and only 5% exist in chimpanzee. HSat3 shows the same split. The rare k-mers that distinguish one copy of an array from the others are mostly variants private to one person, which is what makes them usable for genotyping centromeres and useless for dating them. The default tracks of the site therefore measure the age of the sequence vocabulary, and a switch restricts them to single-copy k-mers, which measure the conservation of the locus; the two are meant to be read side by side.

![**Sharing and age strata by copy number in CHM13.** Top: fraction of 31-mers present in each genome, by the number of copies of the 31-mer in CHM13, for five region classes. Bottom: age strata of the same bins; the numbers above the bars give each bin's share of the class.](../fig/fig_multiplicity.png){width=100%}

## Stratigraphy of the centromeres

Figure 8 shows the strata across five centromeres at 10 kb resolution, and Table 8 gives every active HOR array exactly. Two features are invisible in any class average. First, each active array has a flat sharing level per ape across its whole length (chr1: 45% chimpanzee, 44% gorilla, 7% orangutan, 2% siamang over 4.5 Mb, with fluctuations of a few points), stepping to other levels at the array boundaries. The age of an active array is a property of the array, as homogenization predicts, not of a position within it. Second, the layers outside the active array (monomeric and diverged alpha-satellite, the HSat blocks) are spikier and mostly more shared, including with orangutan and siamang: on chr8 the monomeric layer at 43.6 to 44.2 Mb keeps 60 to 80% of its 31-mers in the apes while the active array beside it keeps 23 to 27%; on chr17 the older layers at 21.8 to 23.3 Mb sit at 40 to 80% while the 3.6 Mb active array holds 53% in chimpanzee, 32% in gorilla, 18% in orangutan and 5% in siamang. This is the layered-expansion picture of alpha-satellite evolution [3, 12] seen through exact k-mers: the youngest, most homogenized layer is the least shared.
...
Figure 9 shows the strata along every chromosome at 100 kb resolution. The human-specific fraction picks out the centromeres, the acrocentric short arms (chromosomes 13, 14, 15, 21 and 22), the HSat3 block of chromosome 9 and the HSat2 blocks of chromosomes 1 and 16; sharing with siamang falls to zero in the same places.
